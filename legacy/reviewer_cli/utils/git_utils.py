...
def fetch_pull_request(repo_root: Path, number: int, remote: str = "origin") -> str:
    """Fetch a PR's head into a local ref and return that ref.

    This is the only network call the review side makes. Everything after it —
    the diff, every file read, every grep — is local, which is what keeps the
    sandboxed fs_server in play and the GitHub API out of the hot path.
    """
    ref = f"reviewer/pr-{number}"
    _run_git(["fetch", "--force", remote, f"pull/{number}/head:{ref}"], repo_root)
    return ref


def get_pr_diff(repo_root: Path, base_ref: str, head_ref: str) -> str:
    """The diff GitHub shows on the *Files changed* tab.

    Three dots, not two. ``base...head`` diffs from the merge base to the head,
    so it shows only what the PR changed. ``base..head`` would additionally show
    everything that landed on the base branch since the fork point as if the PR
    had reverted it — and every line number computed from it would be wrong.
    """
    return _run_git(["diff", f"{base_ref}...{head_ref}"], repo_root)


def get_pr_files(repo_root: Path, base_ref: str, head_ref: str) -> list[str]:
    output = _run_git(["diff", "--name-only", f"{base_ref}...{head_ref}"], repo_root)
    return [line for line in output.splitlines() if line]
